...
class AnchorLoader_dff(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
                 feat_stride=16, anchor_scales=(8, 16, 32), anchor_ratios=(0.5, 1, 2), allowed_border=0,
                 aspect_grouping=False):
        """
        ...
        :return: AnchorLoader
        """
        super(AnchorLoader_dff, self).__init__()
    ...
    def get_batch_individual(self):
        ...
        all_data = dict()
        for key in self.data_name:
            all_data[key] = tensor_vstack([batch['data'][key] for batch in rst])
    ...
    def parfetch(self, iroidb):
        # get testing data for multigpu:only for batch=1
        data, label = get_rpn_pair_batch(iroidb)
